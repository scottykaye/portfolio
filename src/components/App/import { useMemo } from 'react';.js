...
// Create a function that handles all the same ternaries for us

export default function App(props) {
  const router = useRouter();
  const [currentTheme, setCurrentTheme] = useLocalStorage(light, light);

  const logoBackgroundColor = currentTheme === light ? '#000000' : '#ffffff';
  const logoPrimaryColor = currentTheme === light ? '#ffffff' : '#000000';

  function toggleTheme() {
    setCurrentTheme((prevTheme) => (prevTheme === light ? light : dark));
  }

  // Value we provide to context
